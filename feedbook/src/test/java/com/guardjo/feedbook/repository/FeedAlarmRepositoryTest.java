...
class FeedAlarmRepositoryTest {
        ...
        for (int i = 0; i < TEST_SIZE; i++) {
            if (i == 0) {
                TEST_DATA.add(FeedAlarm.builder()
                        ...
                        .args(new AlarmArgs(account2.getId(), account2.getNickname()))
                        .feed(feed2)
                        .build()
                );
            } else {
                TEST_DATA.add(FeedAlarm.builder()
                        .feed(feed1)
                        .args(new AlarmArgs(account1.getId(), account2.getNickname()))
    ...
    void test_saveNewFeedAlarm() {
        FeedAlarm newFeedAlarm = FeedAlarm.builder()
                ...
                .args(new AlarmArgs(TEST_ACCOUNTS.get(0).getId(), TEST_ACCOUNTS.get(0).getNickname()))
                .build();
